import { Microphone, MicrophoneSlash, PhoneDisconnect, X } from "@phosphor-icons/react";
import React, { useState } from "react";
import user01 from "../assets/userImages/user_01.png";
import user02 from "../assets/userImages/user_02.png";

function AudioRoom({ showAudioRoom, handleToggleShowAudioRoom }) {
    ...
    return (
        <div
            className={`${showAudioRoom ? "block" : "hidden"} fixed top-0 left-0 h-full w-full z-999999 bg-black/70 flex items-center justify-center`}
        >
            <div className="bg-white dark:bg-smokyBlack w-full max-w-sm rounded-lg px-4 py-4 md:px-8 md:py-8 space-y-6">
                {/* Header */}
                <div className="flex items-center justify-between text-black dark:text-white">
                    <h3 className="font-medium cursor-default">Audio Room</h3>
                    <button
                        className="hover:text-primary"
                        onClick={handleToggleShowAudioRoom}
                    >
